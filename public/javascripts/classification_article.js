$(function () {
    ...
    limit=7;
    function addarticle() {
        $.post('../classification_article_backstage',{
                'limit':limit
            },
            function (data, status) {
                for (var i = 0; i < data.length; i++) {
                    var content_article = $("<li>" +
                        ...
                        "<a class='read_all' href='article_detail.html'>阅读全文>></a>" +
                        "</p>" +
                        "</div>" +
                        "<div class='foot_span'>" +
                        "<ul>" +
            ...
            function (data,status) {
                $(".click_rank ul li").each(function (i) {
                    if(data[i].article_title.length<=14){
                        $(this).children("a").text(data[i].article_title);                        $(this).children("a").attr("title",data[i].article_title);
